...
def get_cashfree_order_status(order_id):
    ...
    try:
        ...
        response = requests.get(f"{CASHFREE_API_URL}/orders/{order_id}", headers=headers, timeout=30)
...
def create_cashfree_order(plan_id, amount, telegram_user_id, telegram_username, payment_for=None, protected_number=None):
    ...
    try:
        ...
        print(f"Creating Cashfree order: {order_id}, Amount: {amount}")
        
        response = requests.post(f"{CASHFREE_API_URL}/orders", json=payload, headers=headers, timeout=30)
        
        print(f"Cashfree Response Status: {response.status_code}")
        
        if response.status_code == 200:
            result = response.json()
            # Try to get payment link from various possible fields
            payment_link = result.get("payment_link") or result.get("payment_url")
            
            # If payment_session_id is returned, construct checkout URL
            if not payment_link and result.get("payment_session_id"):
                if CASHFREE_ENV.upper() == "PRODUCTION":
                    payment_link = f"https://cashfree.com/checkout/{result.get('payment_session_id')}"
                else:
                    payment_link = f"https://sandbox.cashfree.com/pg/checkout/{result.get('payment_session_id')}"
...
def cashfree_webhook():
    ...
    try:
        # Get signature from headers (support multiple header formats)
        ...
        # Extract order_id from various possible structures
        order_id = None
        payment_id = None
        
        # Try different paths to get order_id
        ...
        # Try different paths to get payment_id
        if isinstance(data, dict):
        ...
        # Verify payment status using Cashfree Order Status API (fallback verification)
