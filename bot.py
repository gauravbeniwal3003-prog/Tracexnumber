...
def payment_screenshot_handler(message):
    ...
    try:
        try:
            ...
            print(f"Admin group screenshot forward failed: {forward_error}")
            try:
                bot.forward_message(ADMIN_ID, message.chat.id, message.message_id)
            except Exception as dm_forward_error:
                print(f"Admin DM screenshot forward failed: {dm_forward_error}")
...
"""
    ...
    elif call.data in ["admin_add", "admin_remove", "admin_ban", "admin_unban", "admin_broadcast", "admin_stats", "admin_transactions", "admin_back", "admin_giveaway", "admin_deactivate_unlimited"]:
        if user_id != ADMIN_ID:
            return
        ...
        elif call.data == "admin_deactivate_unlimited":
            user_states[user_id] = "admin_deactivate_unlimited"
            msg = bot.send_message(call.message.chat.id, "🧨 *DEACTIVATE UNLIMITED*\n\nFormat:\n`user_id`\n\nExample: `123456789`\n\nType /cancel to abort", reply_markup=cancel_button(), parse_mode='Markdown')
            bot.register_next_step_handler(msg, process_admin_deactivate_unlimited)
...
📅 {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')}
    ...
    markup.add(InlineKeyboardButton("🧨 DEACTIVATE UNLIMITED", callback_data="admin_deactivate_unlimited"))
    markup.add(
