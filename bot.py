...
Version: 6.1.0 - New Pricing, Removed Auto-Delete & Cashfree
"""
...
PLAN_CONFIG = {
    ...
    "u1d": {"amount": 50, "credits": 0, "unlimited_minutes": 1440, "payment_for": "unlimited", "label": "1 Day Unlimited"},
    "u1w": {"amount": 250, "credits": 0, "unlimited_minutes": 10080, "payment_for": "unlimited", "label": "7 Days Unlimited"},
    "u1m": {"amount": 800, "credits": 0, "unlimited_minutes": 43200, "payment_for": "unlimited", "label": "30 Days Unlimited"},
...
active_identity_sessions = set()
active_identity_sessions_lock = threading.Lock()
active_ifsc_sessions = set()
active_ifsc_sessions_lock = threading.Lock()
...
def call_generic_lookup_api(url, params=None):
...
def call_telegram_lookup_api(username):
    ...
    try:
        ...
        from urllib.parse import urlencode
        url = f"{TELEGRAM_LOOKUP_API_URL}?{urlencode(params)}"
...
def call_identity_lookup_api(aadhar_number):
    ...
    try:
        ...
        from urllib.parse import urlencode
        url = f"{IDENTITY_LOOKUP_API_URL}?{urlencode(params)}"
        return call_generic_lookup_api(url)
    except Exception as e:
...
def call_ifsc_lookup_api(ifsc_code):
    ...
    try:
        ...
        from urllib.parse import urlencode
        url = f"{IFSC_LOOKUP_API_URL}?{urlencode(params)}"
        return call_generic_lookup_api(url)
    except Exception as e:
...
"""
    ...
    output = format_telegram_lookup_result(parsed_result, username_clean, user_id, unlimited_active, unlimited_expiry)
...
def build_daily_report_text(stats_snapshot):
    ...
    if not top:
        ...
        for i, row in enumerate(top, 1):
            ...
            lines.append(f"{i}. {display} | ID `{row.get('user_id')}` | `{row.get('searches', 0)}` lookups | 📱 `{row.get('number_searches', 0)}` | 💬 `{row.get('telegram_searches', 0)}` | 🆔 `{row.get('identity_searches', 0)}` | 🏦 `{row.get('ifsc_searches', 0)}` | 💎 `{row.get('credits_used', 0)}`")
    lines.append("━━━━━━━━━━━━━━━━")
    return "\n".join(lines)
...
@app.route('/')
def home():
    return "TraceX Bot Running - Cashfree Removed"
...
━━━━━━━━━━━━━━━━
🎯 *FEATURES*
• Instant Number Lookup (5 Credits)
• Telegram ID Lookup (12 Credits)
• Identity/Aadhar Lookup (15 Credits)
• IFSC Bank Lookup (20 Credits)
• Fast Response
...
def process_admin_add(message):
    ...
    try:
        ...
        bot.reply_to(message, "❌ Invalid format!\nCredits: `user_id/@username credits`\nUnlimited: `user_id/@username u1d/u1w/u1m`", parse_mode='Markdown')
...
if __name__ == "__main__":
    ...
    print("   • Identity/Aadhar Lookup (15 Credits)")
    print("   • IFSC Bank Lookup (20 Credits)")
    print("   • Updated Costs: Number=5, Telegram=12")
    print("   • Removed API Branding")
    print("   • Keyboard Buttons for Main Menu")
    print("   • Removed Auto-Delete Messages")
    print("   • Removed Cashfree Integration")
    print("   • Updated Pricing:")
    print("     • 40 Credits - ₹20")
    print("     • 120 Credits - ₹50")
    print("     • 400 Credits - ₹100")
    print("     • 1 Day Unlimited - ₹50")
    print("     • 7 Days Unlimited - ₹250")
    print("     • 30 Days Unlimited - ₹800")
    print("=" * 50)
