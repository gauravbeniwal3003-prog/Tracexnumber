...
API_URL = os.getenv("API_URL", "https://techvishalboss.com/apibuy/public/lookup.php")
API_KEY = os.getenv("API_KEY", "TVB_Y9T032")
...
if CASHFREE_ENV.upper() == "PRODUCTION":
...
def get_user(telegram_user_id):
    """Get or create user from Supabase telegram_users table"""
    try:
...
def update_user_credits(telegram_user_id, new_credits):
    ...
    try:
        ...
        print(f"Update credits error: {e}")
        return False
...
def add_credits(telegram_user_id, amount):
    ...
    try:
        ...
        if user:
            ...
            update_user_credits(telegram_user_id, new_credits)
            return new_credits
        return None
    except Exception as e:
...
def deduct_credit(telegram_user_id):
    """Deduct 1 credit if user has credits and no active unlimited plan"""
    try:
        user = get_user(telegram_user_id)
        if not user:
            return False
        ...
        if unlimited_expiry:
            ...
            if expiry_date > datetime.now(timezone.utc):
                return True  # Unlimited active, no deduction
        
        # Check credits
        credits = user.get('credits', 0)
        if credits > 0:
            update_user_credits(telegram_user_id, credits - 1)
            return True
        return False
    except Exception as e:
...
def increment_total_searches(telegram_user_id):
    """Increment total_searches counter"""
    try:
        user = get_user(telegram_user_id)
        if user:
...
def get_cached_result(phone_number):
    """Get cached lookup result from search_results table"""
    try:
...
def save_cached_result(phone_number, raw_data):
    """Save lookup result to cache using upsert to avoid duplicates"""
    try:
        # Check if exists
...
def is_user_banned(telegram_user_id):
    ...
    try:
        response = supabase.table("telegram_users").select("is_banned").eq("telegram_user_id", telegram_user_id).execute()
        if response.data and len(response.data) > 0:
            return response.data[0].get('is_banned', False)
        return False
    except Exception as e:
        print(f"Check banned error: {e}")
        return False
...
def get_all_users():
    """Get all non-banned users for broadcast/giveaway"""
    try:
...
def add_giveaway_credits(credits):
    """Add credits to all users for giveaway"""
    try:
        users = get_all_users()
        success_count = 0
...
def create_cashfree_order(plan_id, amount, telegram_user_id, telegram_username, payment_for=None, protected_number=None):
    ...
    try:
        ...
        response = requests.post(CASHFREE_API_URL, json=payload, headers=headers, timeout=30)
        ...
        if response.status_code == 200:
            ...
            if payment_link:
                ...
                print(f"Cashfree response missing payment link: {result}")
                return None, None
        else:
...
def get_cashfree_order_status(order_id):
    ...
    try:
        ...
        response = requests.get(f"{CASHFREE_API_URL}/{order_id}", headers=headers, timeout=30)
...
def format_lookup_result(result, phone, user_id, unlimited_active=False, unlimited_expiry=None):
    """Format lookup result from API JSON response"""
...
def cashfree_webhook():
    """Handle Cashfree payment webhook with verification via status API"""
    try:
        # Parse webhook data
        ...
        # Verify payment status using Cashfree API (secure verification)
        order_data = get_cashfree_order_status(order_id)
        
        if not order_data:
            print(f"Could not verify order {order_id} status")
            return jsonify({"error": "Verification failed"}), 400
        
        # Check if payment is successful
        if order_data.get('order_status') in ['PAID', 'SUCCESS', 'COMPLETED']:
            # Process payment success
            process_payment_success(order_id, payment_id, order_data)
...
def main_menu_markup(current_user_id=None):
    ...
    # Show admin button only if current user is admin
...
""", reply_markup=markup, parse_mode='Markdown')
    ...
    if cached_result:
        ...
        # Increment total searches for successful result
        if not unlimited_active:
...
"""
    ...
    if result and result.get('results'):
        ...
        # Save to cache (store full JSON, not formatted text)
...
📱 Number: `{phone}`
👤 Name Found: `{first_name}`
📊 Records: `{len(result.get('results', {})) if isinstance(result.get('results'), dict) else len(result.get('results', []))}`
🕐 Time: {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S')}
"""
        try:
            bot.send_message(ADMIN_CHANNEL_ID, log_message, parse_mode='Markdown')
...
"""
        ...
        # Send search log to admin channel (no credit deduction)
        log_message = f"""
🔍 *SEARCH LOG (No Data)*
━━━━━━━━━━━━━━━━
👤 User: {message.from_user.first_name}
...
def handle_plan_selection(call):
    ...
    order_id, payment_link = create_cashfree_order(
        plan_id=plan_id,
        amount=amount,
...
🔎 Total Searches
`{user.get('total_searches', 0) if user else 0}`

🛡️ Account Status
`{'ACTIVE ✅' if not (user and user.get('is_banned')) else 'BANNED ❌'}`
