...
Version: 11.0.5 - Fixed No Data Detection
"""
...
def has_valid_number_results(result):
    """
    Check if the result contains actual valid data (not "No data found" or empty responses)
    """
    ...
    if 'results' in result:
        ...
        if isinstance(api_results, dict):
            # Check if any value in results is a non-empty dict with meaningful data
            for key, value in api_results.items():
                if isinstance(value, dict):
                    # Skip if the dict is empty or only contains empty strings
                    if value:
                        # Check if any value in the dict has actual content
    ...
    # Check individual fields that indicate valid data
    ...
    for key, value in result.items():
        ...
        if isinstance(value, dict) and value:
            ...
            if has_valid_number_results(value):
                return True
...
def has_valid_telegram_results(result):
    """
    Check if the Telegram lookup result contains actual valid data
    """
    if not isinstance(result, dict):
        return False
    ...
    if 'results' in result:
        api_results = result.get('results')
        if isinstance(api_results, dict):
            # Check for Telegram Match data
            telegram_match = api_results.get('Telegram Match', {})
...
def is_no_data_response(result):
    """
    Check if the response is a "No data found" response
    """
    if not isinstance(result, dict):
        return False
    ...
    # Check if results is empty or has no data
    if 'results' in result:
        api_results = result.get('results')
        if isinstance(api_results, dict):
...
""", reply_markup=markup, parse_mode='Markdown')
    ...
    # Check if it's a "No data found" response BEFORE deducting credits
...
"""
    ...
    if has_valid_number_results(result):
        ...
        # No valid data found but no explicit "no data" message
        if not unlimited_active:
            if not deduct_credits(user_id, NUMBER_LOOKUP_COST):
                safe_edit_message(message.chat.id, loading_msg.message_id, "❌ *Failed to deduct credit. Please try again.*", parse_mode='Markdown')
                remove_active_session(user_id)
...
No information found for this Telegram username.
...
"""
    ...
    # Extract telegram_id for protection check
    telegram_id = None
...
@app.route('/')
def home():
    return "TraceX Bot Running - Version 11.0.5 - Fixed No Data Detection!"
...
if __name__ == "__main__":
    ...
    print("=" * 60)
    print("🔍 FIXED: No Data Detection")
    print("   • Credits are NOT deducted for 'No data found' responses")
    print("   • Proper validation of API responses")
    print("   • Better error handling for empty results")
    print("=" * 60)
